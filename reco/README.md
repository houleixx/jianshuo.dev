...
## 路由
- `POST /reco/engage/<shareId>` body `{action:"view"|"finish"|"like"|"report", on?:bool}` → `{ok}`(like 另带 `{liked}`)。
  `report` = 举报,和 like/finish 一样走 engage,每用户去重、一次性、不可撤销(无 `on=false` 分支)。
...
`action ∈ view|finish|like|report`(表结构不变,`report` 只是多一种 action 值)。

## 排序
`score = (1 + view*1 + finish*4 + like*3 + reply*5 + report*(-9)) / (ageHours + 2)^1.5`,再做作者打散(同作者每多出现一次分 ×0.5)。权重见 `src/ranking.js` 的 `W`,可调。
**举报是负权重**:`report:-9`(= 一个举报抵 3 个负点赞)。一次举报就能把冷启动帖压成负分沉底,但不会让一两个举报抹掉一篇真有互动的热帖(防单点举报滥用)。
