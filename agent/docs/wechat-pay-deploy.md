...
商户平台的「产品介绍 URL」使用 `https://voicedrop.cn/agent/wechat-pay/product`。
该页面随 Agent Worker 发布，支持公开 GET / HEAD 访问，不依赖登录、支付配置或数据库；测试环境使用测试 Agent 的域名和相同路径。部署前不要把尚未生效的地址提交审核。

页面内容位于 `src/wechat-pay-product.js`，说明当前 ¥19.9/月、每期 200 算力、提前三天申请扣款、算力有效期和取消方式。修改套餐或扣款规则时，应同步更新此页、客户端与获批模板。模板自动生成的“到期当天扣费”文案需与当前提前申请规则对齐。
