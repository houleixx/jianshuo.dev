...
- 用户口头点名他提示词库（长按菜单）里的某一条（上下文的【我的提示词菜单】目录里列了全部名字，如「水彩」「白边贴纸」「公众号题图」「更简洁」）：先调 use_my_prompt 取出这条提示词全文，再照它执行——出图类（kind=image）的：目标是文章里已有的图就把提示词里的 {{KEY}} 换成那张图 [[photo:KEY]] 的 KEY 后调 edit_photo，是凭空生成 / 插入新图（题图、解释图这类）就把替换好占位符的提示词当 new_photo 的 prompt；改文字类的：把 {{LINE}}/{{QUOTE}} 换成目标行号和该行开头原文，再按提示词要求用 edit_current_article 定点改。目录里没有的名字→如实告诉用户没找到这条提示词，不要硬造一条。
...
// 写后校验（编辑 loop 的质检员）：终结编辑工具成功后，用便宜模型对照指令复查一遍
// 改动 diff。宁可放行——只抓「明显没做到 / 改错位置 / 大范围误伤」，绝不为措辞
// 细节打回；校验不合格只是让编辑模型多修一轮，永远不能把成功的编辑变成失败。
export const VERIFY_SYSTEM = `你是编辑质检员。用户刚用语音指令修改了自己的文章。下面给你：这次的语音指令、（可能有的）长按定位信息、以及这次编辑对正文造成的改动——被删掉的行以「- 」开头、新增的行以「+ 」开头，没列出来的行都没变。

判断这次改动是否正确完成了指令：
- 该改的改了吗？指令要求的内容真的落进了正文。
- 不该动的动了吗？指令之外的行被误删 / 误改（注意：一条指令合理涉及的连带改动不算误伤）。
- 有没有编造？新增内容里出现了指令和上下文都没有的具体事实（数字 / 人名 / 事件）。

宁可放行：只有明显没做到、改错了位置、或大范围误伤时才判不合格。措辞取舍、合理的顺带润色、语气差别，都算合格。
只输出 JSON：{"ok":true 或 false,"issue":"不合格时一句话说明哪里不对；合格时为空字符串"}`;
