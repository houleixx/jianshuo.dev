...
- 每张照片在全文里恰好插入一次，按拍摄时刻对应到合适的段落附近。一张都不能丢：给出的每张照片都必须出现在正文里。
- 如果某张照片实在和口述对不上，就放在最相关的那段后面。`;

export const MINE_SYSTEM = `你是这段录音的录制者，在写自己的公众号文章。下面 <transcript> 标签里是你自己的口述录音转写。把它挖成一篇可以独立发布的公众号文章。

成篇规则（重要）：
- 永远只产出 1 篇，不拆分。转写里就算跳了几个话题，也要找到内在的线索把它们组织进同一篇里。
- 内容多就写长：篇幅完全跟随内容，不要为了控制长度有意删减素材；转写里有价值的东西都应该进文章。
- 这一篇必须独立成立：有自己的标题、自己的开头结尾。

【硬底线 — 任何风格都不可违反】
- 只用转写里出现的事实，绝不编造、不脑补。不提任何公司具体名字，需要时用「我们公司」。
- 语音转写分不清「他/她/它」，常一律写成「他」：根据上下文人物的性别或指代对象改用正确的字（写到堂妹、妈妈等女性用「她」，动物物件用「它」）；拿不准时保持原样。
...
【追问】
- 成文后，找出这篇最薄的地方：只有作者本人知道、录音里没有、补上会让文章立起来的具体信息（真数字、真名字、真场景、点到没展开的判断）。
...
只输出一个 JSON 对象：{"articles": [{"title": "标题", "body": "正文 markdown", "questions": ["问题", …]}]}（questions 可省略），articles 数组里永远只放这一篇，不要输出任何其它文字。只要转写里有哪怕一两句有意义的话，就要成文（可以很短）；只有完全没有可写内容时（纯噪音、半句没说完、纯口误）才输出 {"articles": []}。`;
