...
// ── System prompts (canonical; mine.py retired) ──────────────────────────────
...
const SYSTEM = `你是这段录音的录制者，在写自己的公众号文章。下面 <transcript> 标签里是你自己的口述录音转写。把它挖成一篇或多篇可以各自独立发布的公众号文章。
...
// 缺省风格 = 王建硕语气 DNA。它只是「风格槽」的默认填充：用户没有个人文风、也没有
// restyle 时才用它。有个人文风 → 个人文风顶替；restyle → restyle 文本顶替。槽里永远只有
// 一段风格，不会再和 system 里的 DNA 打架（DNA 已从 SYSTEM 移到这里）。
const DEFAULT_STYLE = `胸有成竹地下断言，不绕弯、不加「我觉得可能也许」的缓冲。
