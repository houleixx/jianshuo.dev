...
function fixture() {
  ...
  const fetcher=async(url,init)=>{
    ...
    if(url.endsWith('/papay/preentrustweb')) return new Response(wechatV2Xml({
      return_code:'SUCCESS',result_code:'SUCCESS',appid:'app',mch_id:'mch',pre_entrustweb_id:'pure-prepay',
      miniprogram_username:'gh_sign',miniprogram_path:'pages/sign?pre_entrustweb_id=pure-prepay',
...
it('closes only a verified expired unpaid APP order before creating a new pure signing contract',async()=>{
  const f=fixture();await f.call('checkout','{}');const previous=f.txn();
  const r=await f.call('contract','{}',{},NOW+31*60000);expect(r.status).toBe(200);
  const next=await r.json();expect(next.contract_code).not.toBe(previous.contract_code);
  expect(next.pre_entrustweb_id).toBe('pure-prepay');
  expect(f.db.prepare('SELECT status,failure_code FROM wechat_txn WHERE out_trade_no=?').bind(previous.out_trade_no).first())
    .toEqual({status:'failed',failure_code:'CLOSED'});
  expect(f.db.prepare('SELECT status FROM wechat_sub WHERE contract_code=?').bind(previous.contract_code).first().status).toBe('expired');
  expect(f.requests.filter(r=>r.url.endsWith('/close'))).toHaveLength(1);
  expect(f.requests.filter(r=>r.url.endsWith('/app-with-contract'))).toHaveLength(1);
  expect(f.grants()).toBe(0);
});
it.each(['SUCCESS','USERPAYING'])('does not replace an APP order with state %s when reopening pure signing',async state=>{
  const f=fixture();await f.call('checkout','{}');f.setState(state);
  expect((await f.call('contract','{}',{},NOW+31*60000)).status).toBe(409);
  expect(f.requests.some(r=>r.url.endsWith('/close')||r.url.endsWith('/papay/preentrustweb'))).toBe(false);
  expect(f.db.prepare('SELECT COUNT(*) n FROM wechat_sub').first().n).toBe(1);
});
it('does not close a still-valid APP order just to start pure signing',async()=>{
  const f=fixture();await f.call('checkout','{}');
  expect((await f.call('contract','{}',{},NOW+1000)).status).toBe(409);
  expect(f.requests.some(r=>r.url.endsWith('/close'))).toBe(false);
});
