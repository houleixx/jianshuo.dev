...
function fixture() {
 ...
 WECHAT_PAY_API_V2_KEY:'test-key',WECHAT_PAY_CALLBACK_BASE_URL:'https://example.test'};
 ...
 const fetcher=async(url,init)=>{
  ...
  if(url.endsWith('/contractorder'))return xml({prepay_id:'prepay-'+p.out_trade_no,trade_type:'APP',contract_result_code:'SUCCESS'});
  ...
  if(url.endsWith('/orderquery') && db.prepare("SELECT payment_kind FROM wechat_txn WHERE out_trade_no=?").bind(p.out_trade_no).first()?.payment_kind==='app')return xml({out_trade_no:p.out_trade_no,trade_state:'NOTPAY',trade_type:'APP'});
 ...
 const call=async(path,body=null,h={})=>{
  ...
  return handleWechatPayRoute(url,new Request(url,{method:body===null?'GET':'POST',headers:{Authorization:'Bearer anon_unittesttoken_abcdefghijklmnop','CF-Connecting-IP':'198.51.100.7',...h},...(body===null?{}:{body})}),env,fetcher,at);
 };
 ...
 const pay=async()=>{
  const body=await notify('app-pay-notify',{appid:'app',out_trade_no:first().out_trade_no,trade_type:'APP',
    transaction_id:'wx-'+first().out_trade_no,total_fee:1,fee_type:'CNY',time_end:beijing(at).replace(/[- :]/g,'')});
  expect(body.return_code).toBe('SUCCESS');
 };
...
it.each(['pay-first','sign-first'])('joins V2 payment and XML agreement in either order: %s',async order=>{
...
it.each([40])('cancel then reopen at day %i keeps prior paid coverage and allows one new V2 payment',async days=>{
...
it('restore-only intent never becomes a paid checkout after coverage expires',async()=>{
 ...
 expect((await response.json()).error).toBe('coverage-changed');expect(f.requests.filter(r=>r.url.endsWith('contractorder'))).toHaveLength(1);
});
