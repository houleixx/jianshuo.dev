...
describe("magicForItem（item_id 精确解析魔法数字）", () => {
  ...
  it("导入件走 importedFrom（码属原作者），码活着才认", async () => {
    const e = seeded({
      ...
      "shares/4563566": sharedDoc(),
    });
    expect(await magicForItem(e, OWN_SCOPE, "imp1")).toBe("4563566");
...
// ── groupPath —— 分享带分组（2026-07-19）：铸码/写穿记下作者树里的分组路径 ──────
// 数组格式（树两级封顶所以至多一段）是给未来多层留的；导入侧只消费第一段。
describe("groupPath —— 铸码/写穿带分组路径", () => {
  it("系统组内的项铸码 → 副本 groupPath=[组名]；公开预览 GET 也透出", async () => {
    const e = makeEnv();
    const { code } = await (await post(e, { id: SYS_ITEM })).json();
    const doc = JSON.parse(e.FILES._store.get(`shares/${code}`));
    expect(doc.groupPath).toEqual(["改写这段"]);
    const req = new Request(`https://jianshuo.dev/agent/prompt-share/${code}`);
    const r = await handlePromptShareRoutes(new URL(req.url), req, e);
    expect((await r.json()).groupPath).toEqual(["改写这段"]);
  });

  it("顶层自建项铸码 → 副本没有 groupPath 字段（老行为逐字节不变）", async () => {
    const e = makeEnv();
    await putPrompts(e, [{ id: "p_solo0001", type: "action", label: "独行侠", prompt: "独来独往。", appliesTo: ["text"] }]);
    const { code } = await (await post(e, { id: "p_solo0001" })).json();
    expect("groupPath" in JSON.parse(e.FILES._store.get(`shares/${code}`))).toBe(false);
  });

  it("自建组里的项铸码 → groupPath 用组 label；挪出组再保存 → 写穿副本 groupPath 消失", async () => {
    const e = makeEnv();
    const act = { id: "p_act00001", type: "action", label: "改标题", prompt: "把标题改得更抓人。", appliesTo: ["text"] };
    await putPrompts(e, [{ id: "p_grp00001", type: "group", label: "我的菜单", children: [act] }]);
    const { code } = await (await post(e, { id: "p_act00001" })).json();
    expect(JSON.parse(e.FILES._store.get(`shares/${code}`)).groupPath).toEqual(["我的菜单"]);
    // 作者把它拖到顶层：整树 PUT → syncActiveShares 写穿刷新 → groupPath 跟着消失
    await putPrompts(e, [act, { id: "p_grp00001", type: "group", label: "我的菜单", children: [] }]);
    const doc = JSON.parse(e.FILES._store.get(`shares/${code}`));
    expect("groupPath" in doc).toBe(false);
    expect(doc.instruction).toBe("把标题改得更抓人。");
  });

  it("resolvePromptShare 清洗 groupPath：trim、非字符串/空段丢弃；全非法 = 字段缺失", async () => {
    const e = makeEnv({ "shares/7654321": sharedDoc({ groupPath: ["  菜单 ", 42, "", null] }) });
    expect((await resolvePromptShare(e, "7654321")).groupPath).toEqual(["菜单"]);
    const e2 = makeEnv({ "shares/7654321": sharedDoc({ groupPath: [42, ""] }) });
    expect("groupPath" in (await resolvePromptShare(e2, "7654321"))).toBe(false);
  });
});
