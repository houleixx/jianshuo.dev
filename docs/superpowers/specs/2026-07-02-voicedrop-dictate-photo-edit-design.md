...
用户对着一条带图的笔记口述「把图二变成一张广告」，VoiceDrop 的编辑 agent 把它蒸馏成图像编辑 prompt，调用 paint.jianshuo.dev 出图，约 1 分钟后把新图**原地替换**回笔记里图二的位置。计费每张 **4.2 算力**（≈0.025 USD）。
...
edit_photo 工具处理器（agent/src/tools.js，同步、在 turn 内完成）：
   ...
   5. 返回 agent：「🎨 正在把图二改成广告，约 1 分钟完成」（pending 文案）
...
paint 回调 → POST /agent/paint-callback（agent/src/index.js 新路由）：
   ...
   3. status=done：fetch(result_url) → env.FILES.put(scope+newKey, 广告图字节) → debit 4.2 算力
...
- 处理器（ctx 提供 env/scope/articleKey/token/origin/editId/articleIndex）：
  1. `ensureAccount` + 余额预检 `balance_uy >= imageCostUY()`；不足 → `{ error: "算力不足，生成一张图 4.2 算力，请充值" }`。
  ...
  6. 成功 → `{ ok: true, message: "🎨 正在把图片改成…，约 1 分钟完成" }`（不是 TERMINAL 工具，agent 可继续/结束）。
...
- usage.js 新增：`export const IMAGE_SUANLI = 4.2;` 和 `export function imageCostUY() { return suanliToUY(IMAGE_SUANLI); }`（按算力计价，避免 FX 漂移；= `round(4.2/23*1e6)` = 182609 微元 ≈ ¥0.183 ≈ $0.025）。
...
- **制作中（金棕）**：暖纸渐变底 `#F3EEE4→#ECE4D6`、一道扫光（横扫高光，~2.8s 循环）、轻浮的相机图标（`#C98A2E`）、文「正在制作中」（`#8A7B60`）、三个交错呼吸点（`#C98A2E`）、角标文案「约 1 分钟完成」（设计稿写的「预计 10 秒内完成」按此改）。方形缩略尺寸即可（PhotoTile 是 1:1）。
...
## 7. 测试
- **usage.js**：`imageCostUY()` == 182609（`suanliToUY(4.2)` = round(4.2/23×1e6)）。
...
- **端到端（手动，真机+真 Codex）**：一条带图笔记口述「把图X变成广告」→ 看到 pending → 约 1 分钟后原地替换 → 算力余额 -4.2。
