...
- **权限**：`--sandbox workspace-write` —— 工作区内可写可跑命令，区外只读
...
浏览器 ── https://codex.jianshuo.dev
  ...
  → 每条消息 spawn：codex exec --json --sandbox workspace-write -C /opt/codex-agent/workspace
```
...
- 调用参数：`codex exec --json --sandbox workspace-write -C /opt/codex-agent/workspace`
  ＋ `-c sandbox_workspace_write.network_access=true`（沙箱内允许联网：查资料、装包）
...
- 手测清单：新会话问答、续聊上下文保持、跑命令写文件（限定在 workspace 内验证）、
  区外写被拒、断开取消、密码错 401、手机 Safari 可用
